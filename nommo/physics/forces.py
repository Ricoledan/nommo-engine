...
def calculate_forces_vectorized(
    ...
    for pair_idx in prange(len(neighbor_pairs)):
        ...
        if r < cutoff and r > MIN_DISTANCE:
            ...
            force_mag, potential = lennard_jones_force_shifted(
                r, epsilon, sigma, cutoff
            )
    ...
    def setup_parameters(
        ...
        for i in range(n_types):
            for j in range(n_types):
                self.epsilon_matrix[i, j] = np.sqrt(
                    epsilon_values[i] * epsilon_values[j]
                )
...
class CoulombicForce:
    ...
    def calculate(
        ...
        for i, j in neighbor_pairs:
            dr, r = minimum_image_distance(
                positions[i], positions[j], box_size
            )
