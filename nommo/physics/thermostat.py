...
from nommo.physics.constants import ENERGY_CONVERSION, KB_KJMOL_K
...
@njit
def calculate_temperature(velocities: np.ndarray, masses: np.ndarray) -> float:
    """
    ...
    temperature = 2 * kinetic_energy / (n_dof * KB_KJMOL_K) if n_dof > 0 else 0.0

    return temperature


@njit
def rescale_velocities(velocities: np.ndarray, current_temp: float, target_temp: float) -> None:
    """
...
class Thermostat(ABC):
    ...
    @abstractmethod
    def apply(self, velocities: np.ndarray, masses: np.ndarray, dt: float) -> float:
        """
...
class BerendsenThermostat(Thermostat):
    ...
    def __init__(self, target_temperature: float, coupling_time: float = 0.1):
        """
    ...
    def apply(self, velocities: np.ndarray, masses: np.ndarray, dt: float) -> float:
        ...
        lambda_factor = np.sqrt(1 + (dt / self.tau) * (self.T_target / T_current - 1))
    ...
    def _initialize_velocities(self, velocities: np.ndarray, masses: np.ndarray):
...
class VelocityRescaleThermostat(Thermostat):
    ...
    def __init__(self, target_temperature: float, interval: int = 10):
        """
...
class AndersenThermostat(Thermostat):
    ...
    def __init__(self, target_temperature: float, collision_frequency: float = 0.1):
        """
...
class AdaptiveThermostat(Thermostat):
    ...
    def __init__(
        self, target_temperature: float, equilibration_steps: int = 1000, tolerance: float = 10.0
    ):
        """
    ...
    def apply(self, velocities: np.ndarray, masses: np.ndarray, dt: float) -> float:
        ...
        if (
            self.step_count < self.equilibration_steps
            or abs(T_current - self.T_target) > self.tolerance
        ):
...
def create_thermostat(
    thermostat_type: str, target_temperature: float, **kwargs
) -> Thermostat | None:
    """
