...
class Particle:
    ...
    _kinetic_energy_cache: float | None = field(default=None, init=False, repr=False)
    ...
    def can_bond_with(self, other: "Particle") -> bool:
        ...
        """
        return (
            len(self.bonds) < self.max_bonds
            and len(other.bonds) < other.max_bonds
            and other.id not in self.bonds
...
class ParticlePool:
    ...
    def acquire(self, **kwargs) -> Particle:
        ...
        Returns:
            Configured particle
        """
        particle = self._pool.pop() if self._pool else Particle()
