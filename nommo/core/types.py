...
class ParticleTypeConfig(BaseModel):
    ...
    lj_epsilon: Optional[float] = Field(
        default=None, gt=0, description="LJ potential well depth (kJ/mol)"
    )
    lj_sigma: Optional[float] = Field(
        default=None, gt=0, description="LJ zero-crossing distance (nm)"
    )
...
class ReactionRule(BaseModel):
    ...
    catalyst: Optional[str] = Field(default=None, description="Catalyst particle type")
...
class PhysicsParameters(BaseModel):
    ...
    timestep: float = Field(
        default=0.001, gt=0, le=0.1, description="Integration timestep (ps)"
    )
    ...
    adaptive_timestep: bool = Field(
        default=False, description="Enable adaptive timestep"
    )
    max_force: float = Field(
        default=1e6, gt=0, description="Maximum allowed force (kJ/mol/nm)"
    )
...
class ChemistryParameters(BaseModel):
    ...
    bond_energy: float = Field(
        default=20.0, gt=0, description="Default bond energy (kJ/mol)"
    )
    bonding_distance: float = Field(
        default=0.4, gt=0, description="Maximum bonding distance (nm)"
    )
    ...
    reaction_rules: List[ReactionRule] = Field(
        default_factory=list, description="Reaction rules"
    )
...
class ThermodynamicsParameters(BaseModel):
    ...
    temperature: float = Field(
        default=300.0, gt=0, le=10000, description="System temperature (K)"
    )
    pressure: Optional[float] = Field(
        default=None, gt=0, description="System pressure (bar)"
    )
    ...
    energy_amount: float = Field(
        default=0.0, ge=0, description="Energy input amount (kJ/mol)"
    )
    energy_interval: int = Field(
        default=100, gt=0, description="Steps between energy inputs"
    )
...
class UniverseParameters(BaseModel):
    ...
    checkpoint_interval: int = Field(
        default=1000, gt=0, description="Steps between checkpoints"
    )
    ...
    def validate_composition(self) -> "UniverseParameters":
        ...
        for ptype in self.initial_composition.keys():
...
class SimulationConfig(BaseSettings):
    ...
    visualization_backend: str = Field(
        default="matplotlib", description="Visualization backend"
    )
