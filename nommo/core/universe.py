...
from nommo.physics.integrator import AdaptiveTimestepIntegrator, VelocityVerletIntegrator
from nommo.physics.thermostat import calculate_temperature, create_thermostat
from nommo.utils.logging import get_logger, get_performance_logger, get_simulation_logger
...
class Universe:
    ...
    def __init__(self, parameters: UniverseParameters):
        ...
        self.history: deque[UniverseMetrics] = deque(maxlen=parameters.history_buffer_size)

        self._energy_accumulator = 0.0
        self._last_checkpoint = 0

        logger.info(f"Created universe '{parameters.name}' with {len(self.particles)} particles")
    ...
    def _setup_physics(self):
        ...
        self.spatial_index = SpatialIndex(
            self.box_size, params.cutoff_distance, params.neighbor_skin, use_cell_list=True
        )
        ...
        if params.adaptive_timestep:
            self.integrator = AdaptiveTimestepIntegrator(
                params.timestep, self.box_size, self.params.box.boundary_type
            )
        else:
        ...
        for ptype, count in self.params.initial_composition.items():
            ...
            for _ in range(count):
                ...
                sigma_v = velocity_from_temperature(
                    self.params.thermodynamics.temperature, config.mass
                )
    ...
    def step(self) -> UniverseMetrics:
        ...
        potential_energy = self.integrator.step(
            positions, velocities, forces, masses, force_calculator, self.params.physics.timestep
        )
        ...
        if self.thermostat:
            current_temp = self.thermostat.apply(velocities, masses, self.params.physics.timestep)
            for i, p in enumerate(self.particles):
                p.velocity = velocities[i]
        else:
    ...
    def _process_bonds(self, temperature: float):
        ...
        for pair in self.spatial_index.get_pairs():
            ...
            if (
                distance < self.bonding_distance
                and p1.can_bond_with(p2)
                and np.random.random() < self.params.chemistry.reaction_probability
                and p1.add_bond(p2)
            ):
                bonds_formed += 1
    ...
    def _apply_energy_source(self):
        ...
        if energy_type == EnergySource.CONSTANT:
            ...
            energy_type == EnergySource.PULSES
            and self.tick % self.params.thermodynamics.energy_interval == 0
        ):
            energy_per_particle = self.params.thermodynamics.energy_amount / len(self.particles)
    ...
    def _calculate_metrics(self, potential_energy: float, temperature: float) -> UniverseMetrics:
        ...
        metrics.avg_bonds_per_particle = total_bonds / len(self.particles) if self.particles else 0
    ...
    def run(
        self,
        ticks: int,
        callback: Callable[[UniverseMetrics], None] | None = None,
        stop_condition: Callable[[UniverseMetrics], bool] | None = None,
        ...
        for i in range(ticks):
            ...
            if (i + 1) % 1000 == 0:
                ...
                logger.info(f"Progress: {i+1}/{ticks} ticks, " f"{ticks_per_second:.1f} ticks/s")

        elapsed = time.perf_counter() - start_time
        logger.info(f"Simulation complete: {len(metrics_list)} ticks in {elapsed:.2f}s")
