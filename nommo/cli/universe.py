...
def delete(ctx, universe_id, force):
    ...
    if not force and not Confirm.ask(f"Delete universe '{found['filename']}'?"):
        return
...
def _interactive_setup(name: str, description: str) -> UniverseParameters:
    ...
    _temperature = FloatPrompt.ask("Temperature (K)", default=300.0)
    _timestep = FloatPrompt.ask("Timestep (ps)", default=0.001)

    console.print("\n[bold]Box Parameters:[/bold]")
    _box_size = FloatPrompt.ask("Box size (nm)", default=10.0)
    ...
    for i in range(n_types):
        ...
        particle_types[ptype_name] = ParticleTypeConfig(name=ptype_name, mass=mass, radius=radius)
